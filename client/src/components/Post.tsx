...
const Post: React.FC<PostProps> = ({ isReply, replyable, data: { _id, author, body, board, time, liked: intialLiked, likes: initialLikes, replies, ref } })=> {
	...
	return (
		<Container onClick={()=>{ if (!isReply) window.open(`/post/${_id}`); else window.open(`/post/${ref}`)}}>
			<PostHeader>
				...
				<PostTime>{moment(time).fromNow()} in </PostTime>
				<PostBoard href={`/board/${board}`} >{board}</PostBoard>
			</PostHeader>
